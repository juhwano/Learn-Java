...
자료 구조의 하나로, 자료의 삽입과 삭제가 끝에서만 일어나는 선형 목록.
밑이 막힌 통을 세워 놓은 것으로 생각하면 된다. 

자료의 삽입, 삭제가 일어나는 곳을 스택의 톱(top)이라고 하며 
자료를 스택에 넣는 것을 푸시(push), 스택에서 자료를 꺼내는 것을 팝(pop)이라고 한다. 

스택에서는 나중에 들어간 자료가 먼저 꺼내지는 후입 선출(LIFO) 방식을 사용한다. 
스택은 주로 어떤 내용을 기억시켰다가 다시 신속히 이용하고자 할 때 사용되며, 컴퓨터 알고리듬에서 자주 쓰이는 중요한 자료 구조이다.
